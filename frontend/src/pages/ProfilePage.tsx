...
type ShelterInfo = {
    ...
    phoneVerified?: boolean; imageUrl?: string;
};
type Animal = {
...
const UserInfoTab = () => {
    ...
    if (!info) return <div
        style={{color: 'var(--text-muted)', fontSize: 14, padding: 20, textAlign: 'center'}}>Завантаження...</div>;
    ...
    return (
        <div style={section}>
            {!hasPhone && <PhoneRequiredNotice
                message="Для подачі заявок на усиновлення необхідно вказати та підтвердити номер телефону."/>}
...
const UserRequestsTab = () => {
    ...
    if (loading)
        return <div
            style={{textAlign: 'center', padding: '40px 20px', color: 'var(--text-muted)'}}>Завантаження...</div>;
...
const ShelterAnimalsTab = () => {
    ...
    return (
        <div style={{display: 'flex', flexDirection: 'column', gap: 16}}>
            {!shelterHasPhone && (
                <PhoneRequiredNotice
                    message="Вкажіть номер телефону у вкладці «Про притулок» перед тим, як додавати тварин."/>
            )}
...
export const ProfilePage = () => {
    ...
    const tabs = isShelter
        ? [{label: 'Про притулок', icon: '🏠'}, {label: 'Заявки', icon: '📬'}, {
            label: 'Тварини',
            icon: '🐾'
        }, {label: 'Мої події', icon: '📅'}]
        : [{label: 'Про мене', icon: '👤'}, {label: 'Уподобані', icon: '♡'}, {
            label: 'Мої заявки',
            icon: '📋'
        }, {label: 'Мої події', icon: '📅'}];
